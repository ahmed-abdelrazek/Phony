    ...
    public class ServiceVM : CommonBase
    {
            ...
            else
            {
                ...
                if (isvalidmoney)
                {
                    using (var db = new UnitOfWork(new PhonyDbContext()))
                    {
                        ...
                        db.Complete();
                        await ServicesMassage.ShowMessageAsync("تمت العملية", $"تم شحن خدمة {DataGridSelectedService.Name} مبلغ {clientpaymentamount} جنية بنجاح");
        ...
        private void DoEditService(object obj)
        {
            using (var db = new UnitOfWork(new PhonyDbContext()))
            {
                ...
                Services.Add(s);
                DataGridSelectedService = null;
                ServicesMassage.ShowMessageAsync("تمت العملية", "تم تعديل الصنف بنجاح");
            }
        }
        ...
        private void DoAddService(object obj)
        {
            using (var db = new UnitOfWork(new PhonyDbContext()))
            {
                ...
                db.Complete();
                Services.Add(s);
                ServicesMassage.ShowMessageAsync("تمت العملية", "تم اضافة الصنف بنجاح");
            }
        }
        ...
        {
            var result = await ServicesMassage.ShowMessageAsync("حذف الصنف", $"هل انت متاكد من حذف الصنف {DataGridSelectedService.Name}", MessageDialogStyle.AffirmativeAndNegative);
            if (result == MessageDialogResult.Affirmative)
            {
                ...
                DataGridSelectedService = null;
                await ServicesMassage.ShowMessageAsync("تمت العملية", "تم حذف الصنف بنجاح");
            }
        }
